 ...
 * VALID SUDOKU PUZZLE
 * LANGUAGE: C++
 * Determine whether a given 9x9 sudoku puzzle is valid. A cell can either
 * contain a digit 1-9 or be blank, denoted with "."
 */
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (board[i][j] != ".") {
                    ...
                    if (seen["g-" + to_string(i / 3) + to_string(j / 3) +
                             board[i][j]] > 0) {
                        return false;
                    }
                    seen["r-" + to_string(i) + board[i][j]] = 1;
                    seen["c-" + to_string(j) + board[i][j]] = 1;
                    seen["g-" + to_string(i / 3) + to_string(j / 3) +
                         board[i][j]] = 1;
                }
            }
        }
